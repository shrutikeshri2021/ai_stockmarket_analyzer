...
from utils.alerts_engine import evaluate_alerts, format_alerts_for_notification
from utils.sector_analysis import get_sector_performance
from utils.trade_journal import (
    add_trade, load_journal, get_journal_stats, clear_journal
)
from backtesting.backtest import run_backtest
from dashboard.beginner_mode import render_beginner_dashboard
...
with st.sidebar:
    ...
    # ── NEW: Price Alert System ──────────────────────────────────
    st.markdown("---")
    st.markdown("#### 🚨 Price Alerts")
    enable_price_alert = st.checkbox("Enable Price Alert", value=False)
    price_target = 0.0
    price_direction = "above"
    if enable_price_alert:
        price_target = st.number_input(
            "Target Price ($)", min_value=0.0, value=0.0,
            step=1.0, format="%.2f", key="price_target"
        )
        price_direction = st.selectbox(
            "Alert when price goes", ["above", "below"], key="price_dir"
        )

    enable_rsi_alert = st.checkbox("Enable RSI Alert (oversold)", value=False)
    rsi_threshold = 30.0
    if enable_rsi_alert:
        rsi_threshold = st.slider(
            "RSI Threshold", min_value=10, max_value=50,
            value=30, step=5, key="rsi_thresh",
            help="Alert when RSI falls below this value"
        )

    enable_signal_alert = st.checkbox(
        "Alert on BUY Signal", value=False,
        help="Show alert when composite signal = BUY"
    )
    # ── END: Price Alert System ──────────────────────────────────
...
# =====================================================================
# 🧑‍🎓 BEGINNER / ADVANCED MODE TABS
# =====================================================================
tab_beginner, tab_advanced = st.tabs(["🎓 Beginner Learning Mode", "📈 Advanced Trading Mode"])

with tab_beginner:
    render_beginner_dashboard(
        selected_ticker, current_price, predicted_price,
        signal_data["signal"], hist_df, sentiment,
    )
...
with tab_advanced:
    ...
    # =====================================================================
    # � PRICE ALERT SYSTEM (NEW)
    # =====================================================================
    # Get the latest RSI value from the indicator DataFrame
    _latest_rsi = df_ind.iloc[-1].get("RSI", None)
    if _latest_rsi is not None:
        try:
            _latest_rsi = float(_latest_rsi)
        except (TypeError, ValueError):
            _latest_rsi = None

    # Evaluate all user-defined alert conditions
    _triggered_alerts = evaluate_alerts(
        current_price=current_price,
        rsi_value=_latest_rsi,
        signal=sig,
        price_alert_enabled=enable_price_alert,
        price_target=price_target,
        price_direction=price_direction,
        rsi_alert_enabled=enable_rsi_alert,
        rsi_threshold=float(rsi_threshold),
        signal_alert_enabled=enable_signal_alert,
    )

    # Display triggered alerts as prominent banners
    if _triggered_alerts:
        st.markdown('<div class="sec-hdr">🚨 Triggered Alerts</div>',
                    unsafe_allow_html=True)
        for _al in _triggered_alerts:
            if _al["level"] == "success":
                st.success(_al["message"])
            elif _al["level"] == "warning":
                st.warning(_al["message"])
            else:
                st.error(_al["message"])

        # Dispatch alert notifications via email / Telegram if enabled
        _notif_body = format_alerts_for_notification(
            _triggered_alerts, selected_ticker
        )
        if _notif_body:
            if enable_email:
                send_email_alert(
                    f"🚨 AITrade Alerts – {selected_ticker}", _notif_body
                )
            if enable_telegram:
                send_telegram_alert(_notif_body)
    ...
    # =====================================================================
    # 🏢 SECTOR PERFORMANCE DASHBOARD (NEW)
    # =====================================================================
    st.markdown('<div class="sec-hdr">🏢 Sector Performance – Today</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="data-src">
        🏭 Shows the average daily % change for major market sectors using representative stocks.
        Data fetched live from Yahoo Finance.
    </div>
    """, unsafe_allow_html=True)

    with st.spinner("🏢 Scanning sector performance..."):
        _sector_data = get_sector_performance()

    if _sector_data:
        # ── Bar chart ────────────────────────────────────────────────
        _s_names = [f'{s["icon"]} {s["sector"]}' for s in _sector_data]
        _s_vals  = [s["change_pct"] for s in _sector_data]
        _s_colors = ["#00e676" if v > 0 else "#ff1744" for v in _s_vals]

        _fig_sector = go.Figure(go.Bar(
            x=_s_vals, y=_s_names, orientation="h",
            marker_color=_s_colors,
            text=[f"{v:+.2f}%" for v in _s_vals],
            textposition="outside",
            textfont=dict(size=13, color="#c9d1d9"),
        ))
        _fig_sector.update_layout(
            template="plotly_dark", height=55 * len(_sector_data) + 60,
            margin=dict(l=10, r=40, t=10, b=10),
            plot_bgcolor="#0d1117", paper_bgcolor="#0d1117",
            xaxis=dict(title="Avg Daily Change %", zeroline=True,
                       zerolinecolor="#30363d", gridcolor="#21262d"),
            yaxis=dict(autorange="reversed"),
        )
        st.plotly_chart(_fig_sector, key="sector_perf_chart")

        # ── Detail expander ──────────────────────────────────────────
        with st.expander("🔍 View individual stock breakdown per sector"):
            for _sec in _sector_data:
                _lbl_clr = "#00e676" if _sec["change_pct"] > 0 else "#ff1744"
                st.markdown(
                    f"**{_sec['icon']} {_sec['sector']}** — "
                    f"<span style='color:{_lbl_clr}; font-weight:700;'>"
                    f"{_sec['change_pct']:+.2f}%</span> avg "
                    f"({_sec['count']} stocks)",
                    unsafe_allow_html=True,
                )
                if _sec["tickers"]:
                    _tk_cols = st.columns(min(len(_sec["tickers"]), 4))
                    for _ti, _tk in enumerate(_sec["tickers"]):
                        _c = _tk_cols[_ti % len(_tk_cols)]
                        _tc = "#00e676" if _tk["change_pct"] > 0 else "#ff1744"
                        _c.markdown(
                            f"<div style='background:#161b22; padding:8px 12px; "
                            f"border-radius:8px; text-align:center; margin:2px; "
                            f"border:1px solid #30363d;'>"
                            f"<b style='color:#58a6ff;'>{_tk['ticker']}</b><br>"
                            f"<span style='color:#fff;'>${_tk['price']:,.2f}</span><br>"
                            f"<span style='color:{_tc}; font-weight:700;'>"
                            f"{_tk['change_pct']:+.2f}%</span></div>",
                            unsafe_allow_html=True,
                        )
                st.markdown("")
    else:
    ...
    # =====================================================================
    # 📝 TRADING JOURNAL – PAPER TRADING LOG (NEW)
    # =====================================================================
    st.markdown('<div class="sec-hdr">📝 Trading Journal – Paper Trading Log</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="data-src">
        💼 Log your paper trades manually to track performance. Data is saved locally in
        <code>data/trade_journal.csv</code> and persists across sessions.
    </div>
    """, unsafe_allow_html=True)

    _tj_col_form, _tj_col_stats = st.columns([1, 1])

    # ── Left column: Trade Entry Form ─────────────────────────────
    with _tj_col_form:
        st.markdown("**➕ Log a New Trade**")
        with st.form(key="trade_form", clear_on_submit=True):
            _tj_ticker = st.text_input(
                "Ticker Symbol", value=selected_ticker,
                help="Stock ticker (e.g. AAPL, TSLA)"
            )
            _tj_c1, _tj_c2 = st.columns(2)
            _tj_buy = _tj_c1.number_input(
                "Buy Price ($)", min_value=0.01, value=100.0,
                step=0.50, format="%.2f"
            )
            _tj_sell = _tj_c2.number_input(
                "Sell Price ($)", min_value=0.01, value=105.0,
                step=0.50, format="%.2f"
            )
            _tj_qty = st.number_input(
                "Quantity (shares)", min_value=1, value=10, step=1
            )

            # Preview P/L before submitting
            _tj_preview_pl = round((_tj_sell - _tj_buy) * _tj_qty, 2)
            _tj_preview_clr = "#00e676" if _tj_preview_pl >= 0 else "#ff1744"
            st.markdown(
                f"<div style='text-align:center; padding:6px; "
                f"background:#161b22; border-radius:8px; margin:4px 0;'>"
                f"Estimated P/L: <b style='color:{_tj_preview_clr}; "
                f"font-size:1.1rem;'>${_tj_preview_pl:+,.2f}</b></div>",
                unsafe_allow_html=True,
            )

            _tj_submit = st.form_submit_button("💾 Save Trade", use_container_width=True)

        if _tj_submit:
            if _tj_ticker.strip():
                _saved = add_trade(_tj_ticker, _tj_buy, _tj_sell, _tj_qty)
                if "error" not in _saved:
                    st.success(
                        f"✅ Trade logged: {_saved['quantity']} shares of "
                        f"**{_saved['ticker']}** — P/L: **${_saved['profit_loss']:+,.2f}**"
                    )
                    st.rerun()  # refresh to show updated history
                else:
                    st.error(f"❌ Failed to save trade: {_saved['error']}")
            else:
                st.warning("⚠️ Please enter a valid ticker symbol.")

    # ── Right column: Stats Dashboard ────────────────────────────
    with _tj_col_stats:
        _tj_stats = get_journal_stats()
        st.markdown("**📊 Performance Summary**")

        _tj_s1, _tj_s2, _tj_s3 = st.columns(3)
        _tj_s1.metric("📊 Total Trades", _tj_stats["total_trades"])
        _pl_clr = "🟢" if _tj_stats["total_profit"] >= 0 else "🔴"
        _tj_s2.metric(
            f"{_pl_clr} Total P/L",
            f"${_tj_stats['total_profit']:+,.2f}",
        )
        _tj_s3.metric("🏆 Win Rate", f"{_tj_stats['win_rate']}%")

        _tj_s4, _tj_s5, _tj_s6 = st.columns(3)
        _tj_s4.metric("✅ Winning", _tj_stats["winning_trades"])
        _tj_s5.metric("❌ Losing", _tj_stats["losing_trades"])
        _tj_s6.metric("💰 Avg P/L", f"${_tj_stats['avg_profit']:+,.2f}")

        if _tj_stats["total_trades"] > 0:
            _tj_s7, _tj_s8 = st.columns(2)
            _tj_s7.metric("🚀 Best Trade", f"${_tj_stats['best_trade']:+,.2f}")
            _tj_s8.metric("📉 Worst Trade", f"${_tj_stats['worst_trade']:+,.2f}")

    # ── Trade History Table ───────────────────────────────────────
    _tj_journal = load_journal()

    if not _tj_journal.empty:
        with st.expander(f"📜 View Trade History ({len(_tj_journal)} trades)", expanded=False):
            # Style the table: green for profit, red for loss
            def _tj_color_pl(val):
                try:
                    v = float(val)
                    if v > 0:
            ...
            _tj_display = _tj_journal.copy()
            _tj_display.index = range(1, len(_tj_display) + 1)
            _tj_display.index.name = "#"

            _tj_styled = _tj_display.style.map(
                _tj_color_pl, subset=["profit_loss", "profit_pct"]
            ).format({
                "buy_price": "${:,.2f}",
                "sell_price": "${:,.2f}",
                "profit_loss": "${:+,.2f}",
            ...
            st.dataframe(_tj_styled, width='stretch', height=300)
        ...
        st.caption("📝 No trades logged yet. Use the form above to log your first paper trade.")
